...
fn main() {
    let evl = EventLoopBuilder::new().build();
    let (wnd,dsp) = SimpleWindowBuilder::new().build(&evl);
    let egui = EguiGlium::new(&dsp,&wnd,&evl);
    ...
    evl.run(move |event,window_target|{
        //let next_frame_time = std::time::Instant::now() +
        //    std::time::Duration::from_nanos(16_666_667);
        match event {
            WindowEvent{event,..}=>match event{
//                event::WindowEvent::CloseRequested =>,                 event::WindowEvent::KeyboardInput {input,..}=> {
